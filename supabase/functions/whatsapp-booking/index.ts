...
import knStrings from './kn-strings.json' with { type: 'json' };
...
async function processInbound(body) {
  ...
  try {
    ...
    await sendText(phone, MSG.en.genericError);
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const processing = processInbound(body);
    if (typeof EdgeRuntime !== 'undefined' && EdgeRuntime.waitUntil) EdgeRuntime.waitUntil(processing);
